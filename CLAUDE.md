...
**`js/attribution.js`** (IIFE) — first-touch attribution for the CRM (Fase 3, [f7b2d2a](../../commit/f7b2d2a)). No cookies/deps from third parties: keeps an anonymous `ddv_sid` (own cookie + localStorage fallback), captures UTM params + referrer + landing page on first visit and keeps them immutable in localStorage, fills hidden inputs on `#contactForm` so they ride along in the Netlify Forms POST, appends `?sid=` to links pointing at `tienda.deyvidev.com` (joins the portfolio session to the store), and fires a `sendBeacon` to the store's `/api/track/whatsapp` on WhatsApp link clicks. Never blocks a click or form submit — all writes are wrapped in try/catch.
